...
export async function runWatcher(
  ...
  const issues = (await jira.searchRecentIssues(projectKey, sinceIso))
    .filter((issue) => !isIgnoredSummary(issue.summary));
  ...
  const allIssues: StoredIssue[] = getStoredIssues().filter((issue) =>
    isOnOrAfterIso(issue.updated, sinceIso) && !isIgnoredSummary(issue.summary)
  );
